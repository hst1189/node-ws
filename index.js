...
const UUID = process.env.UUID || '8cda8d90-9c66-4f59-8f98-d433d6238a8c'; //←★运行哪吒v1时在不同的平台需要改UUID,否则会被覆盖
...
const NAME = process.env.NAME || 'mysite';                 //←★节点服务器名称
...
const GetISP = async () => {
  try {
    ...
    const data = res.data;
    ISP = `${data.country_code}-${data.region}-${data.isp}-${data.timezone}`.replace(/ /g, '_');
  } catch (e) {
    ISP = 'Unknown';
  }
}
